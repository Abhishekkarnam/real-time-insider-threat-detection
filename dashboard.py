...
from insider_threat_detection.real_time_collector import (
    ensure_events_file,
    is_background_collector_running,
    is_psutil_available,
    is_scapy_available,
...
@st.cache_data(ttl=2)
def load_dashboard_data(csv_path: str, file_size: int, modified_ns: int) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def csv_fingerprint(csv_path: Path) -> tuple[int, int]:
...
def ensure_event_data_file() -> None:
    ensure_events_file(DATA_PATH)
...
def build_sidebar(events_df: pd.DataFrame) -> dict[str, object]:
    ...
    backend_available = (
        is_scapy_available() if collector_backend == "scapy" else is_psutil_available()
    )

    if run_collector and not backend_available:
        st.sidebar.error(f"Install {collector_backend} first: python -m pip install -r requirements.txt")
    elif run_collector:
        started = start_background_collector(
            DATA_PATH,
            interval_seconds=5,
        ...
        if started:
            st.cache_data.clear()
        st.sidebar.success(f"Real-time collector is running with {collector_backend}.")
    else:
        if stop_background_collector():
            st.cache_data.clear()
        st.sidebar.info("Real-time collector is paused.")

    if is_background_collector_running():
        st.sidebar.caption("Collector interval: 5 seconds")
    else:
        st.sidebar.caption("Collector is not currently running.")
...
def maybe_refresh_dashboard(auto_refresh: bool) -> None:
    if not auto_refresh:
        return

    interval_ms = int(st.session_state.get("refresh_seconds", 5)) * 1000
    st_autorefresh(interval=interval_ms, key="dashboard_autorefresh")
    st.cache_data.clear()
...
def main() -> None:
    ...
    ensure_event_data_file()
    file_size, modified_ns = csv_fingerprint(DATA_PATH)
    events_df, alerts_df = load_dashboard_data(str(DATA_PATH), file_size, modified_ns)
    controls = build_sidebar(events_df)
    st.session_state["refresh_seconds"] = controls["refresh_seconds"]
    maybe_refresh_dashboard(auto_refresh=bool(controls["auto_refresh"]))
    file_size, modified_ns = csv_fingerprint(DATA_PATH)
    events_df, alerts_df = load_dashboard_data(str(DATA_PATH), file_size, modified_ns)
    ...
    if bool(controls["auto_refresh"]):
        st.info(f"Auto-refresh is running every {int(controls['refresh_seconds'])} seconds.")
    if bool(controls["run_collector"]):
        st.caption(
            "Real-time collection is enabled. "
            f"The {controls['collector_backend']} backend appends new events every 5 seconds."
        )

    if events_df.empty:
        st.warning(
            "No real network event data is available yet. Open a networked application "
            "or run the collector as Administrator for broader Windows visibility."
        )
        return
